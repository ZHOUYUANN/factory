<template>
  <div class="component-setting">
    <SettingHeader v-on="$listeners" title="文本设置" />
    <el-form label-width="80px" label-position="left" size="small">
      ...
      <BInput label="字间距" :num="letterSpacing" :min="0" @change="handleChangeInput('letterSpacing', $event)"/>
      <BInput label="行间距" :num="form.style.lineHeight" :step="0.5" @change="handleChangeInput('lineHeight', $event)"/>
    </el-form>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      letterSpacing: parseUnitNumber(copyObj(this.componentData.style.letterSpacing)),
